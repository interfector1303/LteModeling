...
public:
	eNodeB(uint64_t p_id, Runtime& p_runtime, std::shared_ptr<spdlog::logger> p_logger,
		...
		: m_id(p_id),
		  ...
		  m_PDCCH(std::make_unique<PDCCH>(p_runtime, p_logger, "PDCCH"))
	{
		m_PUCCH->addReciever(this);

		m_runtime.schedule(m_runtime.start() - 2ns, std::bind(&eNodeB::schedule, this, std::placeholders::_1), this);
	}
...
private:
	...
	std::shared_ptr<UlScheduler> ulScheduler;

	// channels
	std::shared_ptr<PUCCH> m_PUCCH;
	std::unique_ptr<PDCCH> m_PDCCH;
};
